  ...
  var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if(xhttp.readyState = 4 && xhttp.status == 200)
			isCustomer = xhttp.responseText;
	};
	xhttp.open("GET", "php/validateCredentials.php?action=is_customer&uname=" + uname + "&pswd=" + pswd, true);
	xhttp.send();
}
  ...
  if(!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    if(confirmation == "CONFIRM") {
      var xhttp = new XMLHttpRequest();
    	...
    	xhttp.open("GET", "php/validateCredentials.php?action=store_customer_info&pharmacy_name=" + pharmacy_name.value + "&address=" + address.value + "&email=" + email.value + "&contact_number=" + contact_number.value + "&username=" + username.value + "&password=" + password.value, true);
    	xhttp.send();
      return true;
    }
  }

...
function updateCustomerDetails() {
  ...
  if(!validateName(pharmacy_name.value, 'pharmacy_name_error'))
    ...
    if(validatePassword(password)) {
      ...
      xhttp.open("GET", "php/validateCredentials.php?action=update_customer_info&pharmacy_name=" + pharmacy_name.value + "&address=" + address.value + "&email=" + email.value + "&contact_number=" + contact_number.value + "&username=" + username.value, true);
      xhttp.send();
      edit(true);
      return true;
    }
    else
      document.getElementById('customer_acknowledgement').innerHTML = "<span class='text-danger'>Invalid Password!</span>";
    return false;
  }
}
...
function checkCustomerPassword(password, error) {
...
function changePassword() {
  ...
  if(!checkCustomerPassword(old_password.value, 'old_password_error'))
    old_password.focus();
